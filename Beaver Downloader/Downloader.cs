...
namespace Beaver_Downloader
{
    public class Downloader : IDownloader
    {
        ...
        public async Task GetFile(string id)
        {
            ...
            int currentByte = int.Parse(file["CurrentByte"].InnerText);
        ...
        /// <returns></returns>
        private async Task<HttpResponseMessage> MakeRequest(string url, HttpMethod method, int currentByte = 0)
        {
            // Create the request
        ...
        /// <param name="currentByte"></param>
        /// <returns></returns>
        private FileStream SetFileStream(string path, int currentByte)
        {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        private async Task StreamToFile(Stream responseStream, FileStream fileStream, int currentByte, string id)
        {
            // Set the buffer size
            ...
            while ((read = await responseStream.ReadAsync(buffer, 0, buffer.Length)) > 0)
            {
                ...
                // Update the currentByte in the xmlFile
